		...
		<div class="data-section">
			...
			<span>
				<table border="1">
					...
					<?php 
						$select_massage="SELECT `id`, `Sento`, `Title`, `Massage`, `Replay`, `CreateDate` FROM `notification` ORDER BY `CreateDate` DESC";
						$run_massage=mysqli_query($conn,$select_massage);
						while ($row_masage=mysqli_fetch_array($run_massage))
						{
							?>
							<tr>
								<td>
									<?php echo $row_masage['Sento'];?>
								</td>
								<td>
									<?php echo $row_masage['Title'];?>
								</td>
								<td>
									<?php echo $row_masage['Massage'];?>
								</td>
								<td>
									<?php echo $row_masage['Replay'];?>
								</td>
