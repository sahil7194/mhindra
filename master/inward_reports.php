				...
				<table style="margin-left: 30px; width: 1160px; margin-top: 20px;" border="1">
					...
					<?php 
						...
						switch ($get_24)
						 {
							case '6':
								$get_data_1="SELECT `id`, `invoiceNumber`, `ItemName`, `Quantity`, `Value`, `serialNumber`, `WorkDoneBy`, `CreateDate`,`warehouse` FROM `inward` ORDER BY `CreateDate` DESC;";
								break;

							case '7':
								$get_data_1="SELECT * FROM `inward` WHERE `invoiceNumber`='$invoice_number_1'  ORDER BY  `CreateDate` DESC;";
								break;

							case '3':
								$get_data_1="SELECT * FROM `inward` WHERE `warehouse`='$warehouse'  ORDER BY  `CreateDate` DESC;";
								break;

							case '4':
								$get_data_1="SELECT * FROM `inward` WHERE `invoiceNumber`='$invoice_number_1' AND `warehouse`='$warehouse'  ORDER BY  `CreateDate` DESC";
								break;
						}
						...
						while ($row_data=mysqli_fetch_assoc($run_data))
						{
							?>
								<tr>
									...
									<td><a target="_blank" href="inward_details.php?id=<?php echo $row_data['invoiceNumber'];?>&warehouse=<?php echo $row_data['warehouse'];?>">Deatils</a></td>
								</tr>
							<?php
						}
					 ?>
