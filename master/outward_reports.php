	...
	<div class="working-section">
		<div class="data-section">
			<span style="margin-left: 90px;">
				<h1 style="text-align: center; font-size: 40px;">Outward Reports</h1>
				<h3 style="margin-left: 90px;">Export	<a href="download_excel_new_outward_report.php">Excel</a></h3>
				...
				<div style=" height: 680px; width: 1200px; overflow: auto;">
				<table style="margin-left: 30px; width: 1160px; margin-top: 20px;" border="1">
					...
					<?php 
						...
						switch ($get_24)
						 {
							case '6':
								$get_data_1="SELECT `id`, `invoiceNumber`, `ItemName`, `Quantity`, `Value`, `serialNumber`, `WorkDoneBy`, `warehouse`, `UOM`, `CreateDate` FROM `outward` ORDER BY `CreateDate` DESC;";
								break;

							case '7':
								$get_data_1="SELECT `id`, `invoiceNumber`, `ItemName`, `Quantity`, `Value`, `serialNumber`, `WorkDoneBy`, `warehouse`, `UOM`, `CreateDate` FROM `outward` WHERE `invoiceNumber`='$invoice_number_1'  ORDER BY  `CreateDate` DESC;";
								break;

							case '3':
								$get_data_1="SELECT `id`, `invoiceNumber`, `ItemName`, `Quantity`, `Value`, `serialNumber`, `WorkDoneBy`, `warehouse`, `UOM`, `CreateDate` FROM `outward` WHERE `warehouse`='$warehouse'  ORDER BY  `CreateDate` DESC;";
								break;

							case '4':
								$get_data_1="SELECT `id`, `invoiceNumber`, `ItemName`, `Quantity`, `Value`, `serialNumber`, `WorkDoneBy`, `warehouse`, `UOM`, `CreateDate` FROM `outward` WHERE `invoiceNumber`='$invoice_number_1' AND `warehouse`='$warehouse'  ORDER BY  `CreateDate` DESC";
								break;
						}

						$get_data_1;
						//$get_data = "SELECT `id`, `invoiceNumber`, `ItemName`, `Quantity`, `Value`, `serialNumber`, `WorkDoneBy`, `CreateDate` FROM `outward` ORDER BY `CreateDate` DESC";
						...
						while ($row_data=mysqli_fetch_assoc($run_data))
						{
							?>
								<tr>
									...
									<td><a href="detials_outward.php?id=<?php echo $row_data['invoiceNumber'];?>&warehoue=<?php echo $row_data['warehouse'];?>">Deatils</a></td>
								</tr>
							<?php
						}
					 ?>
