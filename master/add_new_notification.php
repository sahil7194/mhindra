...
<head>
	...
	<link rel="stylesheet" type="text/css" href="http://localhost/Current Project/mahindra/css/add_notification.css">
	<style type="text/css">
		input[type='submit']
		{
			height: 40px;
...
<body>
	...
	<div class="working-section">
		<div class="data-section">
			<span>
				<h1 style="text-align:center; font-size:40px; margin-bottom:20px;">Add Notification</h1>
						...
						<tr>
							...
							<td>
								<select name="sentto">
									...
									<?php 
										$option_query="SELECT `id`, `FirstName`, `Address`, `MobileNumber`, `Email`, `DateOfBirth`, `username`, `password`, `role` FROM `login_details`";
										$run_option=mysqli_query($conn,$option_query);
										while ($row_option=mysqli_fetch_array($run_option))
										 {
											?>
											<option value="<?php echo $row_option['username'];?>"><?php echo $row_option['username'];?></option>
											<?php
										}
									 ?>
								</select>
				...
				<?php 
					if (isset($_POST['sent'])) 
					{
						...
						$insert_massage="INSERT INTO `notification`(`Sento`, `Title`, `Massage`) VALUES ('$sentto','$title','$massage')";
						$run = mysqli_query($conn,$insert_massage);
						if ($run == true)
						{
